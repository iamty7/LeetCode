...
public class InsertDeleteGetRandomO1 {
	 ...
	 * have the same probability of being returned. Example:
	 * 
	...
	public class RandomizedSet {
		// Your runtime beats 9.24% of java submissions. Runtime 284ms
		...
		public boolean remove(int val) {
			...
			int targetIndex = map.get(val), lastOne = list.get(list.size() - 1);
			list.add(targetIndex, lastOne);
			list.remove(targetIndex + 1);
			list.remove(list.size() - 1);
			map.put(lastOne, targetIndex);
			map.remove(val);
			return true;
		}
